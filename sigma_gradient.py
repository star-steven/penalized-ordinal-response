def sigma_gradient(beta, X, y, sigma,distribution='norm', loc=0, scale=1, a=None, c=None, s=None):
    '''
    ...
    prob_y_gradient_about_fitvalues = upper*upper_gradient_about_fitvalues - lower_gradient_about_fitvalues*lower
    product = prob_y_gradient_about_fitvalues / prob_y
    product = -1.0 * product / (sigma**2)
    deltah = delta(y)
    product = np.array(deltah * product)
    #fitvalues_gradient = X.values
    #prob_yi = [x[x != 0][0] for x in product]
    #gradient = map(lambda x, y: x * y, prob_yi, fitvalues_gradient)
    gradient = np.sum(product.reshape(product.size,))
...
if __name__ == '__main__': #test the function 
    ...
    beta = pd.Series(beta)
    gradient = sigma_gradient(beta=beta,X=X,y=y,distribution='norm', s=1, sigma=2) 
    print(X)
    print(gradient)
